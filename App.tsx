  ...
  <div className="w-20 lg:w-72 bg-slate-950/80 backdrop-blur-xl border-r border-slate-800/50 flex flex-col h-screen sticky top-0 z-30">
    ...
    <nav className="flex-1 p-4 space-y-2 mt-4">
      {[
        ...
        { id: 'tactical', icon: Radar, label: 'Tactical Analysis', desc: 'AI Recommendations' },
      ].map((item) => (
        <button 
          key={item.id}
...
export default function App() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.error(error);
      alert("Failed to generate analysis. Check API Key.");
    } finally {
  ...
  const handleScoutSearch = async () => {
    ...
    try {
        ...
        alert("Scouting engine failed. Check API Key and try again.");
    } finally {
